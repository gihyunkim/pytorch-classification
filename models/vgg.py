...
class VGG(nn.Module):
    def __init__(self, num_blocks, num_classes=100):
        super().__init__()
        self.block1 = self.get_block(3, 64, num_blocks[0])# 32x32
        self.pool1 = nn.MaxPool2d(2, 2)
        self.block2 = self.get_block(64, 128, num_blocks[1])# 16x16
        self.pool2 = nn.MaxPool2d(2, 2)
        self.block3 = self.get_block(128, 256, num_blocks[2])# 8x8
        # self.pool3 = nn.MaxPool2d(2, 2)
        self.block4 = self.get_block(256, 512, num_blocks[3])
        # self.pool4 = nn.MaxPool2d(2, 2)
        self.block5 = self.get_block(512, 512, num_blocks[4])
        # self.pool5 = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(8*8*512, 4096)
        self.fc2 = nn.Linear(4096, 4096)
        self.out = nn.Linear(4096, num_classes)

    def get_block(self, in_ch, out_ch, num_block):
        layers = []
